...
<?php function media_selector_settings_page_callback() {
	wp_enqueue_media();
	// Save attachment ID
    ...
    <form method="POST">
        <div id="options-container">
            ...
            <div class="options">
                ...
                <br />
                <input type="date" id="datepicker" name="startdate-interstitial" value="<?php echo get_option('startdate-interstitial'); ?>" class="startdate-datepicker" />
                <br />
                <label>When would you like this ad to end?</label>
                <br />
                <input type="date" id="datepicker_two" name="enddate-interstitial" value="<?php echo get_option('enddate-interstitial'); ?>" class="enddate-datepicker" />
				<br />
				<label>How many times would you like to see this ad daily?</label>
                <br />
                <select name="daily" class="form-control" id="sel1">
					<option value="1" <?php echo get_option('daily') == '1' ? 'selected' : '' ?>>1</option>
					<option value="2" <?php echo get_option('daily') == '2' ? 'selected' : '' ?>>2</option>
				</select>
				<hr>
				<h3>Dimensions</h3>
				<input type="text" name="interstitial-width" value="<?php echo get_option('interstitial-width'); ?>" placeholder="width" />
				<input type="text" name="interstitial-height" value="<?php echo get_option('interstitial-height'); ?>" placeholder="height" />
				<hr>
				<h3>DFP Code</h3>
				...
				<p><blockquote></blockquote><pre><code>&lt;div id='div-gpt-ad-<b>1502995302683</b>-0' style='height:600px; width:300px;'&gt;
&lt;script&gt;
googletag.cmd.push(function() { googletag.display('div-gpt-ad-1502995302683-0'); });
&lt;/script&gt;
&lt;/div&gt;
</blockquote></code></pre>
				<input type="text" name="interstitial-dfp-code" value="<?php echo get_option('interstitial-dfp-code'); ?>" placeholder="DFP Code" />
				<?php wp_handle_upload( $file, $overrides, $time ); ?>
				<br />
				<hr>
				<input type='hidden' value='0' name='activate_ad'>
				<input class="activate_ad" style="float:left;position: relative;top:14px;" type="checkbox" name="activate_ad" value="<?php echo get_option('activate_ad'); ?>" <?php echo get_option('activate_ad') == '1' ? 'checked' : 'unchecked' ?> />
            	<h3>Activate Ad?</h3>
